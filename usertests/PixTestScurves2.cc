...
ClassImp(PixTestScurves2)

// ----------------------------------------------------------------------
PixTestScurves2::PixTestScurves2(PixSetup *a, std::string name) : PixTest(a, name), 
  fParDac(""), fParNtrig(1), fParDacLo(0), fParDacHi(0), fParDacsPerStep(1), fParNtrigPerStep(1), 
  fAdjustVcal(1), fDumpAll(-1), fDumpProblematic(-1), fDumpOutputFile(-1),fParVwllPrLow(0),
  fParVwllPrHigh(255),fParVwllPrStep(20),fParVwllShLow(0),fParVwllShHigh(255),fParVwllShStep(20) {
  PixTest::init();
  init(); 
}
...
//----------------------------------------------------------
PixTestScurves2::PixTestScurves2() : PixTest() {
  //  LOG(logDEBUG) << "PixTestScurves ctor()";
}

// ----------------------------------------------------------------------
bool PixTestScurves2::setParameter(string parName, string sval) {
  bool found(false);
  ...
  for (unsigned int i = 0; i < fParameters.size(); ++i) {
	...
	fParNtrigPerStep = atoi(sval.c_str()); 
      ...
      if (!parName.compare("vwllprlow")) {
  fParVwllPrLow = atoi(sval.c_str()); 
      }
      if (!parName.compare("vwllprhigh")) {
  fParVwllPrHigh = atoi(sval.c_str()); 
      }
      if (!parName.compare("vwllprstep")) {
  fParVwllPrStep = atoi(sval.c_str()); 
      }
       if (!parName.compare("vwllshlow")) {
  fParVwllShLow = atoi(sval.c_str()); 
      }
      if (!parName.compare("vwllshhigh")) {
  fParVwllShHigh= atoi(sval.c_str()); 
      }
      if (!parName.compare("vwllshstep")) {
  fParVwllShStep = atoi(sval.c_str()); 
      }
...
// ----------------------------------------------------------------------
void PixTestScurves2::setToolTips() {
...
// ----------------------------------------------------------------------
void PixTestScurves2::bookHist(string name) {
  fDirectory->cd(); 
...
//----------------------------------------------------------
PixTestScurves2::~PixTestScurves2() {
  LOG(logDEBUG) << "PixTestScurves2 dtor";
}


// ----------------------------------------------------------------------
void PixTestScurves2::doTest() {
  ...
  bigBanner(Form("PixTestScurves2::doTest() ntrig = %d", fParNtrig));
  scurves();
...
void PixTestScurves2::fullTest() {
  ...
  fParDacsPerStep = -1;   
  fParNtrigPerStep = -1;   
  bigBanner(Form("PixTestScurves2::fullTest() ntrig = %d, dacs/step = %d, ntrig/step = %d", fParNtrig, fParDacsPerStep, fParNtrigPerStep));
  scurves();
  ...
  LOG(logINFO) << "PixTestScurves2::fullTest() done, duration: " << seconds << " seconds";
...
// ----------------------------------------------------------------------
void PixTestScurves2::runCommand(string command) {
...
// ----------------------------------------------------------------------
void PixTestScurves2::scurves() {
  gStyle->SetPalette(1); 
  fStopTest = false;
  fDirectory->cd();
  cacheDacs();

  banner(Form("PixTestScurves2::scurves(%s), ntrig = %d, dacs/step = %d, ntrig/step = %d", 
  ...
  for (int k=fParVwllPrLow; k<fParVwllPrHigh; k=k+fParVwllPrStep) {
    for (int sh=fParVwllShLow; sh<fParVwllShHigh; sh=sh+fParVwllShStep) {
      fApi->setDAC("VwllPr", k);
      fApi->setDAC("VwllSh", sh);
      LOG(logINFO) << "Setting DAC VwllPr to: " << k;
      LOG(logINFO) << "Setting DAC VwllSh to: " << sh;
      string kstring = static_cast<ostringstream*>( &(ostringstream() << k) )->str();
      string shstring = static_cast<ostringstream*>( &(ostringstream() << sh) )->str();
      fOutputFilename = ("SCurveData_VwllPr" + kstring + "_VwllSh" + shstring).c_str();
  ...
  restoreDacs();
  LOG(logINFO) << "PixTestScurves2::scurves() done ";
  dutCalibrateOff();
}


// ----------------------------------------------------------------------
void PixTestScurves2::thrMap() {
  cacheDacs();
  PixTest::update(); 
  fDirectory->cd();
  ...
  LOG(logINFO) << "PixTestScurves2::thrMap() start: " 
  ...
  PixTest::update(); 
  restoreDacs();
  LOG(logINFO) << "PixTestScurves2::thrMap() done ";
  dutCalibrateOff();
...
// ----------------------------------------------------------------------
void PixTestScurves2::adjustVcal() {
